...
(defun arnav/eshell-new (name)
  (+eshell/open-popup "~")
  )
(defun arnav/check-and-open-eshell-new (name)
  (if (and (get-buffer name) (string= (buffer-mode name) "eshell-mode"))
      (progn
        (switch-to-buffer name nil t))
    (arnav/eshell-new name)
    ))

(defun arnav/ivy-eshell-new ()
  (interactive)
  ...
  (ivy-read "Eshell Buffers: " (mapcar #'buffer-name
            ...
            :action '(1
                      ("s" arnav/check-and-open-eshell-new "switch to buffer"))
            ))
#+END_SRC
